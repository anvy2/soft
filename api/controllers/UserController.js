...
module.exports.validate = function (req, res) {
  ...
  con.query(
    'select * from login_logout_log a  where a.user_id=? and a.logged_out_time is null and a.logged_in_time like ?\
  order by a.log_id desc limit 1',
    [id, day + '%'],
    function (err, result) {
...
module.exports.register = function (req, res) {
  ...
  con.query('INSERT INTO users SET ?', users, function (
    error,
    results,
    fields
  ) {
...
module.exports.Login = async (req, res) => {
  ...
  if (tempHash == pass) {
    ...
    const token = jwt.sign(
      { _id: id, _password: tempHash, _key: '@bhijeet@' },
